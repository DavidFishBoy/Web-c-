...
<head>
    ...
    <style>
        ...
        .cart-summary-details li span:last-child {
            font-weight: 600;
            color: #111;
        }
        .cart-summary-details li.total-summary {
            font-size: 18px;
            font-weight: 700;
            color: #111;
        ...
        .cart-summary-details li.total-summary span:last-child {
            font-size: 20px;
            color: #e63946;
        }
        /* HẾT CSS MỚI */
            ...
            <div class="row">
                    ...
                    <c:if test="${empty requestScope.listGH}">
                        <div class="cart-summary text-center py-5">
                            ...
                            <div class="row justify-content-center">
                                <div class_ ="col-lg-6 col-md-6 col-sm-6">
                                        ...
                                        <tr data-price="${o.giaBan}" data-mactsp="${o.maCTSP}">
                                            ...
                                            <td>
                                                <form class="quantity-form">
                                                    ...
                                                    <input type="number" class="item-qty" name="soLuong" value="${o.soLuong}" min="1">
                                                </form>
                                            </td>
                                            <td>
                ...
                <div class="col-lg-4">
                    <div class="cart-summary">
                        <h6>Tóm tắt giỏ hàng</h6>
                        
                        <div class="cart__discount">
                            <h6>Mã giảm giá</h6>
                            <form action="#">
                                <input type="text" placeholder="Nhập mã giảm giá">
                                <button type="submit" class="primary-btn-outline">Áp dụng</button>
                            </form>
                        </div>
                        ...
                        <ul class="cart-summary-details">
                            <li>
                                ...
                                <span id="sub-total-display"><fmt:formatNumber type="number" maxFractionDigits="0" value="${requestScope.tongTien}" /> VNĐ</span>
                            </li>
                            <li>
                            ...
                            <li class="total-summary">
                                <span>Tổng cộng:</span>
                                <span id="grand-total"><fmt:formatNumber type="number" maxFractionDigits="0" value="${requestScope.tongTien}" /> VNĐ</span>
                            </li>
                        </ul>
...
function updateGrandTotal(){
    ...
    // Cập nhật cả 2 trường Tạm tính và Tổng cộng
    $('#sub-total-display').text(formatVND(grandTotal));
...
function updateCartItem(inputElement){
    ...
    if(newQty < 1 || isNaN(newQty)){
        if(confirm("Bạn có muốn xóa sản phẩm này?")) {
            window.location.href = `XoaGioHangServlet?maCTSP=${maCTSP}`;
        } else {
            // Nếu không xóa, khôi phục giá trị cũ (cần lưu giá trị cũ, hoặc đơn giản là đặt lại là 1)
            // Tốt hơn là gọi lại hàm updateCartItem với giá trị là 1
            $input.val(1); // Đặt lại là 1
            updateCartItem($input[0]); // Gọi lại hàm để cập nhật subtotal
        }
...
$(document).ready(function(){
    // Xóa 'input' để tránh gọi AJAX liên tục khi gõ, chỉ gọi khi thay đổi hoặc blur
    $('.item-qty').on('change blur', function(){ 
        updateCartItem(this); 
    });
    
    // Đảm bảo tổng tiền được tính đúng khi tải trang
    updateGrandTotal(); 
});
</script>
</body>
